...
function getAvailableRooms(roomQuantity = 0, dates_array = []) {
  ...
  available_data['roomRate'] = dates_array.map((acc, item) => {
    return {
      date: acc.date,
      price: acc.rate,
    }
...
function handleCreateBooking(params) {
  ...
  try {
    ...
    const booking = {
      ...
      roomPrice: params.roomPrice ? JSON.parse(params.roomPrice) : '0',
...
function saveBooking(booking) {
  ...
  updateRoomStatus(booking.roomPrice.map(e => e.date), ss.getSheetByName('Room Detail'));
}
...
function verifyCaptcha(params) {
  var secretKey = '6LeZdfkqAAAAALcXRGXLE_6l9uQBbmV-_woJ2za9'; // Replace with your reCAPTCHA secret
...
function sendBookingConfirmationEmail(booking) {
              ...
              <div class="email-body">
                  ...
                  <div class="booking-details">
                      ...
                      <strong>ราคาต่อคืน:  </strong>&nbsp;&nbsp;&nbsp;
                          <span><ul>${booking.roomPrice.map((item) => {
    return `<li>${item.date}:&nbsp;&nbsp;&nbsp;&nbsp;฿${Number(item.price).toLocaleString()}</li>`
  }).join('')}</span>
                      </div>
  ...
  ราคาต่อคืน: ฿${booking.roomPrice.map((item) => {
    return `${item.date}: ฿${item.price.toLocaleString()}`
  }).join('\n')}
...
function updateRoomStatus(dateList, sheet) {
  let data = sheet.getRange(2, 4, sheet.getLastRow(), 6).getDisValues();
  for(let i = 0; i < data.length; i++) {
    if(dateList.includes(data[i][0])) {
      let status = data[i][3];
      let remain = data[i][5];
      if(remain == '0' && status != 'CLOSED') {
        sheet.getRange(i + 2, 6).setValue('CLOSED');
      }
      dateList = dateList.filter(e => e != data[i][0]);

   ...
   let data = sheet.getDataRange().getValues();
    for (let i = 1; i < data.length; i++) { // Start from 1 to skip header row
         if (data[i][1] == appId) { // Assuming application_id is in the first column
              return {
                submission_date: data[i][0],
                application_id: data[i][1],
                prefix: data[i][2],
                full_name: data[i][3],
                id_card_number: data[i][4],
                date_of_birth: data[i][5],
                age: data[i][6],
                nationality: data[i][7],
                religion: data[i][8],
                mobile_phone: data[i][9],
                email: data[i][10],
                address_registration: data[i][11],
                address_current: data[i][12],
                occupation: data[i][13],
                business_name: data[i][14],
                address_business: data[i][15],
                business_location_details: data[i][16],
                business_phone: data[i][17],
                years_experience: data[i][18],
                expertise: data[i][19].split(',').map(s => s.trim()),
                delivery_address_type: data[i][20],
                health_status: data[i][21],
                health_chronic_detail: data[i][22],
                health_disability_detail: data[i][23],
                beneficiary_name: data[i][24],
                beneficiary_relationship: data[i][25],
                beneficiary_id_card: data[i][26],
                beneficiary_phone: data[i][27],
                beneficiary_address: data[i][28],
                is_approved: data[i][29],
                copy_id_card: data[i][30],
                copy_house_reg: data[i][31],
                photo_1_inch: data[i][32],
                occupation_proof: data[i][33],
                reviewer_officer: data[i][34],
                approval_date: data[i][35],
                signatures: data[i][36]
              };
         }
    }

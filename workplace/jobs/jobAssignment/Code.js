...
function getScriptURL() {
  return ScriptApp.getService()
    .getUrl()
    .replace("script.google.com/macros", "script.google.com/a/*/macros");
}
  ...
  const user = users.find(
    (u) =>
      String(u.Username).toLowerCase() ===
        String(username || "")
          .trim()
          .toLowerCase() && String(u.Active).toLowerCase() !== "false",
  );
...
function getUploadAuth(jobId) {
  ...
  return {
    ...
    folderId: getJobFolder(jobId).folderId,
  };
}
function getExportImageBase64(fileId, sessionToken) {
  const session = getSession(sessionToken);
  if (!session) throw new Error("Session หมดอายุ กรุณาเข้าสู่ระบบอีกครั้ง");
  ...
  if (!/^image\/(png|jpe?g)$/.test(mimeType)) return null;
  return {
    base64: Utilities.base64Encode(blob.getBytes()),
    mimeType,
    extension: mimeType.includes("png") ? "png" : "jpeg",
  };
}
...
function saveAssignment(payload) {
  ...
  return withScriptLock_(() => {
    ...
    const sheet =
      ss.getSheetByName(CONFIG.ASSIGNMENTS_SHEET) || initSheet_().assignments;
    ...
    const row = HEADERS.Assignments.map((header) =>
      values[header] === undefined ? "" : values[header],
    );
    sheet
      .getRange(sheet.getLastRow() + 1, HEADERS.Assignments.indexOf("ForemanUsernames") + 1)
      .setNumberFormat("@");
...
function updateAssignment(payload) {
  // payload = { "area": "โรงอาหารแลปโก้", "username": "supervisorSCG", "assignDate": "2026-09-21", "job": "ตัดหญ้าสนามและเก็บกวาด", "rank": "A", "owner": "คุณชัยยุทธ (ผู้ตรวจรับงาน SCG #2)", "site": "หัวหน้างาน SCG #2", "assignmentId": "FF-2609002", "foremanUsernames": [], "foremanNames": [] }
  return withScriptLock_(() => {
    assertPayload_(payload, [
      "assignmentId",
      "site",
    ...
    const row = values.findIndex(
      (r, i) => i > 0 && String(r[idCol]) === String(payload.assignmentId),
    );
    if (row < 1) throw new Error("ไม่พบรหัสงาน " + payload.assignmentId);
    const record = Object.fromEntries(
      HEADERS.Assignments.map((header, index) => [header, values[row][index]]),
    );
    if (
      String(record.SupervisorUsername).toLowerCase() !==
    ...
    if (String(record.Status) === "Success")
      throw new Error("งานนี้เสร็จสิ้นแล้ว ไม่สามารถแก้ไขได้");
    const h = Object.fromEntries(
      HEADERS.Assignments.map((header, index) => [header, index + 1]),
    );
    sheet
      .getRange(row + 1, h.Site, 1, 6)
      .setValues([
        [
          payload.site,
          payload.area,
    ...
    sheet.getRange(row + 1, h.ForemanUsernames).setNumberFormat("@");
    sheet.getRange(row + 1, h.ForemanUsernames).setValue(
      (payload.foremanUsernames || []).map(String).join(", "),
    );
    sheet.getRange(row + 1, h.Owner).setValue(payload.owner);
    sheet
      .getRange(row + 1, h.ForemanNames)
      .setValue((payload.foremanNames || []).join(", "));
...
function saveSubmission(payload) {
  return withScriptLock_(() => {
    ...
    const status = String(
      values[row][HEADERS.Assignments.indexOf("Status")] || "",
    );
    if (status === "Success")
      throw new Error("งานนี้เสร็จสิ้นแล้ว ไม่สามารถแก้ไขได้");
    const beforeFileId =
      payload.beforeFileId || values[row][h.BeforeFileId - 1] || "";
    const afterFileId =
      payload.afterFileId || values[row][h.AfterFileId - 1] || "";
    if (!beforeFileId && !afterFileId)
      throw new Error("ต้องมีภาพก่อนหรือภาพหลังอย่างน้อย 1 ภาพ");
    const beforeUrl =
      payload.beforeUrl !== undefined
        ? payload.beforeUrl
        : values[row][h.BeforeUrl - 1] || "";
    const afterUrl =
      payload.afterUrl !== undefined
        ? payload.afterUrl
        : values[row][h.AfterUrl - 1] || "";
    const beforeNote =
      payload.beforeNote !== undefined
        ? payload.beforeNote
        : values[row][h.BeforeNote - 1] || "";
    const afterNote =
      payload.afterNote !== undefined
        ? payload.afterNote
        : values[row][h.AfterNote - 1] || "";
    const nextStatus = beforeFileId && afterFileId ? "Success" : "In progress";
    const supervisorUsername = String(
      values[row][HEADERS.Assignments.indexOf("SupervisorUsername")] || "",
    ).trim();
    sheet
      .getRange(r, h.BeforeFileId, 1, 4)
      .setValues([[beforeFileId, afterFileId, beforeUrl, afterUrl]]);
    ...
    if (status !== "Success" && nextStatus === "Success" && supervisorUsername)
      createNotification_(
        {
...
function ensureAssignmentsSchema_(sheet, lockHeld) {
  ...
  throw new Error(
    "โครงสร้างชีท Assignments ไม่ตรงกับระบบ กรุณาตรวจสอบหัวตารางแถวที่ 1",
  );
}
...
function cancelAssignment(assignmentId, username) {
  return withScriptLock_(() => {
    ...
    const idCol = HEADERS.Assignments.indexOf("AssignmentId");
    const row = values.findIndex(
      (r, i) => i > 0 && String(r[idCol]) === String(assignmentId),
    );
    ...
    if (
      String(values[row][ownerCol]).toLowerCase() !==
      String(actor.Username).toLowerCase()
    )
      throw new Error("คุณไม่มีสิทธิ์ยกเลิกงานนี้");
    const statusCol = HEADERS.Assignments.indexOf("Status") + 1;
    if (String(values[row][statusCol - 1]) === "Cancelled")
      throw new Error("งานนี้ถูกยกเลิกแล้ว");
    if (String(values[row][statusCol - 1]) === "Success")
      throw new Error("งานนี้เสร็จสิ้นแล้ว ไม่สามารถยกเลิกได้");
    const cancelledCol = HEADERS.Assignments.indexOf("CancelledAt") + 1;
    sheet.getRange(row + 1, statusCol).setValue("Cancelled");
    sheet.getRange(row + 1, cancelledCol).setValue(new Date());
    const notificationSheet = ensureNotificationsSheet_(true);
    const notificationValues = notificationSheet.getDataRange().getValues();
    const notificationAssignmentCol =
      HEADERS.Notifications.indexOf("AssignmentId");
    const notificationReadCol = HEADERS.Notifications.indexOf("IsRead") + 1;
    notificationValues.forEach((notification, index) => {
      if (
        index > 0 &&
        String(notification[notificationAssignmentCol]) === String(assignmentId) &&
        String(notification[notificationReadCol - 1]).toLowerCase() !== "true"
      ) {
        notificationSheet.getRange(index + 1, notificationReadCol).setValue(true);
      }
    });
    clearCache_();
    return { ok: true };
...
function getBootstrap_(session) {
  ...
  const users = readRowsDisplay_(ss.getSheetByName(CONFIG.USERS_SHEET), HEADERS.Users);
  ...
  const assignments =
    session.Role === "Foreman"
      ? allAssignments.filter((assignment) =>
          String(assignment.ForemanUsernames || "")
            .split(",")
            .some(
              (username) =>
  ...
  return JSON.stringify({
    ...
    foremen,
    assignments,
    notifications: getNotificationsForUser_(session.Username),
  });
}
...
function markNotificationRead(sessionToken, notificationId) {
  return withScriptLock_(() => {
    ...
    const row = values.findIndex(
      (value, index) =>
        index > 0 &&
        String(value[idCol]) === String(notificationId) &&
        usernamesMatch_(value[recipientCol], session.Username),
    );
  ...
  return withScriptLock_(() => {
    ...
    values.forEach((value, index) => {
      if (
        index > 0 &&
        usernamesMatch_(value[recipientCol], session.Username) &&
        String(value[HEADERS.Notifications.indexOf("IsRead")]).toLowerCase() !==
          "true"
      ) {
...
function getNotificationsForUser_(username) {
  ...
  const rows = readRows_(sheet, HEADERS.Notifications)
    .filter(
      (row) =>
        usernamesMatch_(row.RecipientUsername, username) &&
        String(row.IsRead).toLowerCase() !== "true",
    )
    .sort(
      (a, b) =>
    ...
    .map((row) => ({
      ...row,
      IsRead: String(row.IsRead).toLowerCase() === "true",
    }));
...
function createNotification_(payload, lockHeld) {
  if (!lockHeld)
    return withScriptLock_(() => createNotification_(payload, true));
  const sheet = ensureNotificationsSheet_(true);
  const now = new Date();
  const rowNumber = sheet.getLastRow() + 1;
  const recipientColumn = HEADERS.Notifications.indexOf("RecipientUsername") + 1;
  sheet.getRange(rowNumber, recipientColumn).setNumberFormat("@");
  sheet.getRange(rowNumber, 1, 1, HEADERS.Notifications.length).setValues([[
    Utilities.getUuid(),
    String(payload.recipientUsername ?? "").trim(),
    payload.type,
...
function usernamesMatch_(left, right) {
  const a = String(left || "").trim();
  const b = String(right || "").trim();
  if (a.toLowerCase() === b.toLowerCase()) return true;
  return /^\d+$/.test(a) && /^\d+$/.test(b) && String(Number(a)) === String(Number(b));
}
...
function ensureNotificationsSheet_(lockHeld) {
  ...
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet =
    ss.getSheetByName("Notifications") || ss.insertSheet("Notifications");
  ...
  if (!header.every((value, index) => current[index] === value)) {
    ...
    sheet
      .getRange(1, 1, 1, header.length)
      .setFontWeight("bold")
      .setBackground("#17211b")
      .setFontColor("#ffffff");
  }
  const recipientColumn = header.indexOf("RecipientUsername") + 1;
  const dataRowCount = sheet.getLastRow() - 1;
  if (dataRowCount > 0)
    sheet
      .getRange(2, recipientColumn, dataRowCount, 1)
      .setNumberFormat("@");
  return sheet;
}
...
function getInitialData_() {
  ...
  if (cachedJson) {
    try {
      ...
      if (
        cached &&
        cached.master &&
        Array.isArray(cached.master.supervisors) &&
        cached.dropdowns &&
        Array.isArray(cached.dropdowns.sites) &&
        Array.isArray(cached.dropdowns.owners) &&
        Array.isArray(cached.dropdowns.areas)
      )
        return cached;
  ...
  if (json.length <= CONFIG.INITIAL_DATA_CACHE_MAX_BYTES) {
    cache.put(
      CONFIG.INITIAL_DATA_CACHE_KEY,
      json,
      CONFIG.INITIAL_DATA_CACHE_SECONDS,
    );
  }
  return initialData;
}
...
function buildInitialDataFromMaster_() {
  ...
  rows.forEach((row) => {
    ...
    if (site && !siteSet.has(site)) {
      siteSet.add(site);
      result.dropdowns.sites.push(site);
    }
    if (owner && !ownerSet.has(owner)) {
      ownerSet.add(owner);
      result.dropdowns.owners.push(owner);
    }
    if (area && !areaSet.has(area)) {
      areaSet.add(area);
      result.dropdowns.areas.push(area);
    }

    if (rank && !rankSet.has(rank)) {
      rankSet.add(rank);
      result.dropdowns.ranks.push(rank);
    }
    if (job && !jobSet.has(job)) {
      jobSet.add(job);
      result.dropdowns.jobs.push(job);
    }
    if (rank && job) {
...
function initSheet_() {
  initSheet();
  return {
    assignments: SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
      CONFIG.ASSIGNMENTS_SHEET,
    ),
  };
}
...
function readRowsDisplay_(sheet, headers) {
  if (!sheet || sheet.getLastRow() < 2) return [];
  return sheet
    .getRange(2, 1, sheet.getLastRow() - 1, headers.length)
    .getDisplayValues()
...
function nextAssignmentId_(sheet) {
  ...
  return "FF-" + year + String(count).padStart(3, "0");
}
function requireRole_(username, role) {
  const users = readRowsDisplay_(
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.USERS_SHEET),
    HEADERS.Users,
  );
  ...
  if (!user || user.Role !== role)
    throw new Error("ไม่มีสิทธิ์ดำเนินการสำหรับ role นี้");
  return user;
}
function assertPayload_(payload, fields) {
  if (!payload) throw new Error("ข้อมูลไม่ครบ");
  fields.forEach((f) => {
    if (payload[f] === undefined || payload[f] === null || payload[f] === "")
      throw new Error("ข้อมูลไม่ครบ: " + f);
  });
}
function assertMasterSiteArea_(site, area) {
  ...
  const valid = (master.supervisors || [])
    ...
    .some((item) =>
      (item.areas || []).some(
        (value) => normalizeText_(value) === normalizedArea,
      ),
    );
...
function safeJsonForHtml_(data) {

...
// const SPREADSHEET_ID = "11XvEAj3epwKKYYGMafzcQaXcC7Y25FBW-CIIbrQgIdY"; // Please replace with your actual Spreadsheet ID
const ss = SpreadsheetApp.openById(SPREADSHEET_ID);

const FOLDER_ID = '1xxuY4Wznv9AM1oyhfxbo_-QvBBpq8_Ah'; // Please replace with your actual Folder ID for reports/backups
// const FOLDER_ID = '1CEWlvFURW0X6uRa_uAAPDjyczE5cr329'; // Please replace with your actual Folder ID for reports/backups
...
function getDashboardData() {
  try {
    ...
    const dataToReturn = {
      ...
      subcategories: getProductSubcategories(products),
...
function generateInventoryReportPDF(data) {
  try {
    let response = generateInventoryReportHTML(data);
    if (!response.success) {
      return response; // Return error response from HTML generation
    }
    // Use MimeType constants for clarity
    const blob = Utilities.newBlob(response.html, MimeType.HTML, `Inventory_Report_${Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyyMMdd")}.pdf`).getAs(MimeType.PDF);
...
function generateInventoryReportPrint(data) {
  // This function now just returns the HTML generated by the helper
  logUserAction('Generate Report', 'Inventory Report (Print)');
  let htmlContent = generateInventoryReportHTML(data).html;
...
function generateInventoryReportHTML(data = {}) {
  ...
  try {
    ...
    let filteredDataForTemplate = getInventoryFilteredData(data);
    if (!filteredDataForTemplate.success) {
      return { success: false, message: filteredDataForTemplate.message };
    }
    const filtered = filteredDataForTemplate.filtered;
    ...
    template.startDate = new Date(data.startDate).toLocaleDateString('th-TH', { year: 'numeric', month: 'numeric', day: 'numeric' });
    template.endDate = new Date(data.endDate).toLocaleDateString('th-TH', { year: 'numeric', month: 'numeric', day: 'numeric' });
    ...
    Logger.log("Error generating Inventory HTML: " + e);
    return { success: false, message: "เกิดข้อผิดพลาดในการสร้างรายงาน: " + e.message };
  }
}

function exportInventoryToSheet(data = {}) {
  if (!data || !data.startDate || !data.endDate) {
    return { success: false, message: "กรุณาเลือกช่วงวันที่สำหรับส่งออกข้อมูล" };
  }
  try {
    let response = getInventoryFilteredData(data);
    if (!response.success) {
      return response; // Return error response from data filtering
    }
    const filtered = response.filtered;

    const exportSheet = SpreadsheetApp.create(`Inventory_Export_${Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyyMMdd_HHmmss")}`);
    const sheet = exportSheet.getActiveSheet();
    // Set headers
    const headers = ['รหัสสินค้า', 'ชื่อสินค้า', 'หมวดหมู่', 'ประเภท', 'หน่วยนับ', 'ราคาต่อหน่วย', 'จำนวนคงเหลือ', 'มูลค่ารวม', 'สถานที่จัดเก็บ', 'วันที่อัพเดทล่าสุด'];
    sheet.appendRow(headers);

    // Append data rows
    sheet.getRange(2, 1, filtered.length, headers.length).setValues(
      filtered.map(p => [
        p.code,
        p.name,
        p.category,
    ...
    sheet.getRange(2, 6, filtered.length, 1).setNumberFormat('#,##0.00'); // Price
    sheet.getRange(2, 7, filtered.length, 1).setNumberFormat('#,##0'); // Stock
    sheet.getRange(2, 8, filtered.length, 1).setNumberFormat('#,##0.00'); // Total Value
    sheet.getRange(2, 10, filtered.length, 1).setNumberFormat('dd/mm/yyyy'); // Last Update

    logUserAction('Export', 'Export Inventory to Sheet');
    return { success: true, url: exportSheet.getUrl() };
  } catch (e) {
    Logger.log("Error exporting Inventory to Sheet: " + e);
    return { success: false, message: "เกิดข้อผิดพลาดในการส่งออกไปยังสเปรดชีต: " + e.message };
  }
}

function exportInventoryToCSV(data = {}) {
  if (!data || !data.startDate || !data.endDate) {
    return { success: false, message: "กรุณาเลือกช่วงวันที่สำหรับส่งออกข้อมูล" };
  }
  try {
    ...
    filtered.forEach(p => {
      const row = [
        ...
        (Number(p.stock || 0)) * (Number(p.price || 0)),
        p.location,
        p.lastUpdate ? Utilities.formatDate(new Date(p.lastUpdate), Session.getScriptTimeZone(), 'dd/MM/yyyy') : ''
      ].map(value => `"${String(value).replace(/"/g, '""')}"`); // Escape quotes
      csvRows.push(row.join(',')); // Escape quotes
    });
    const csvContent = csvRows.join('\n');

    logUserAction('Export', 'Export Inventory to CSV Dates: ' + data.startDate + ' to ' + data.endDate + 'Catregory: ' + data.category + ' Subcategory: ' + data.subcategory);
    return { success: true, csvContent: '\ufeff' + csvContent }; // Add BOM for Excel compatibility
  } catch (e) {
    Logger.log("Error exporting Inventory to CSV: " + e);
    return { success: false, message: "เกิดข้อผิดพลาดในการส่งออกเป็นไฟล์ CSV: " + e.message };
  }
}

function getInventoryFilteredData(data) {
  try {
    ...
    Logger.log("Error in getInventoryFilteredData: " + e);
    return { success: false, message: "เกิดข้อผิดพลาด: " + e.message };
  }
}
...
function generateWithdrawalReportHTML(data = {}) {
  ...
  // }
  try {
    const filteredDataForTemplate = getWithdrawalFilteredData(data);
    if (!filteredDataForTemplate.success) {
      return { success: false, message: filteredDataForTemplate.message };
    }
    const {filtered, totalValue, products, transactions} = filteredDataForTemplate;

    const logoBase64 = getBase64Image('https://img2.pic.in.th/pic/be503e88-1d81-4a83-bc3b-a75e914247da.jpg'); // Helper function to get base64 image

    // Use the HTML template
    const template = HtmlService.createTemplateFromFile('WithdrawalReportTemplate');
    template.transactions = filtered;
    template.startDate = new Date(data.startDate).toLocaleDateString('th-TH', { year: 'numeric', month: 'numeric', day: 'numeric' });
    template.endDate = new Date(data.endDate).toLocaleDateString('th-TH', { year: 'numeric', month: 'numeric', day: 'numeric' });
    template.generatedDate = new Date().toLocaleString('th-TH', { year: 'numeric', month: 'numeric', day: 'numeric' }) + '  , ' + new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' }) + ' น.';
    template.categoryFilter = (data.category === 'All' ? 'ทุกหมวด' : data.category) + ' / ' + (data.subcategory === 'All' ? 'ทุกประเภท' : data.subcategory);
    template.transactionTypeFilter = (data.transactionType === 'All' ? 'ทุกรายการ' : data.transactionType === 'in' ? 'รับเข้า' : 'เบิกจ่าย');
...
function exportWithdrawalToSheet(data = {}) {
  if (!data || !data.startDate || !data.endDate) {
    return { success: false, message: "กรุณาเลือกช่วงวันที่สำหรับส่งออกข้อมูล" };
  }
  try {
    let response = getWithdrawalFilteredData(data);
    if (!response.success) {
      return response; // Return error response from data filtering
    }
    const {filtered} = response;

    const exportSheet = SpreadsheetApp.create(`Withdrawal_Export_${Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyyMMdd_HHmmss")}`);
    const sheet = exportSheet.getActiveSheet();
    // Set headers
    const headers = ['วันที่', 'เวลา','ประเภท', 'ชื่อสินค้า', 'จำนวน', 'ราคาต่อหน่วย', 'มูลค่ารวม', 'หมายเหตุ'];
    sheet.appendRow(headers);
    ...
    sheet.getRange(2, 1, filtered.length, headers.length).setValues(
      filtered.map(t => [
        t.date,
        t.time,
        t.transactionType === 'in' ? 'รับเข้า' : 'เบิกจ่าย',
        t.productName,
        Number(t.quantity || 0),
        Number(t.unitPrice.replace(/,/g, '') || 0),
        Number(t.totalCost.replace(/,/g, '') || 0),
        t.note
      ])
    );
    ...
    // Format columns
    sheet.getRange(1, 1, 1, headers.length).setFontWeight('bold');
    sheet.getRange(2, 6, filtered.length, 1).setNumberFormat('#,##0.00'); // Unit Price
    sheet.getRange(2, 5, filtered.length, 1).setNumberFormat('#,##0'); // Quantity
    sheet.getRange(2, 7, filtered.length, 1).setNumberFormat('#,##0.00'); // Total Cost
    sheet.getRange(2, 1, filtered.length, 1).setNumberFormat('dd/mm/yyyy'); // Date

    logUserAction('Export', `Export Withdrawal to Sheet Dates: ${data.startDate} to ${data.endDate} Category: ${data.category} Subcategory: ${data.subcategory} Type: ${data.transactionType}`);
    return { success: true, url: exportSheet.getUrl() };
  } catch (e) {
    Logger.log("Error exporting Withdrawal to Sheet: " + e);
    return { success: false, message: "เกิดข้อผิดพลาดในการส่งออกไปยังสเปรดชีต: " + e.message };
  }
}

function exportWithdrawalToCSV(data = {}) {
  if (!data || !data.startDate || !data.endDate) {
    return { success: false, message: "กรุณาเลือกช่วงวันที่สำหรับส่งออกข้อมูล" };
  }
  try {
    ...
    logUserAction('Export', `Export Withdrawal to CSV Dates: ${data.startDate} to ${data.endDate} Category: ${data.category} Subcategory: ${data.subcategory} Type: ${data.transactionType}`);
    return { success: true, csvContent: '\ufeff' + csvContent }; // Add BOM for Excel compatibility
  } catch (e) {
    Logger.log("Error exporting Withdrawal to CSV: " + e);
    return { success: false, message: "เกิดข้อผิดพลาดในการส่งออกเป็นไฟล์ CSV: " + e.message };
  }
}

function getWithdrawalFilteredData(data) {
  try {
    // Validate dates
    if (!data || !data.startDate || !data.endDate) {
      return { success: false, message: "กรุณาเลือกช่วงวันที่สำหรับรายงาน" };
    ...
    const filtered = transactions.filter(t => {
      ...
      if (data.transactionType && data.transactionType !== 'All') {
        if (t.type !== data.transactionType) return false;
      }
    ...
    const transactionDataForTemplate = filtered
      ...
      return { success: true, filtered: transactionDataForTemplate, totalValue, products, transactions };
  } catch (e) {
    Logger.log("Error in getWithdrawalFilteredData: " + e);
    return { success: false, message: "เกิดข้อผิดพลาด: " + e.message };
  }
}

function generateProjectReportPrint(projectId = "PRJ-2025-005") {
  try {
    const projects = getAllFromCache('projects', projectsSheet, parseProjects);
    const transactions = getAllFromCache('transactions', transactionsSheet, parseTransactions);
    const products = getAllFromCache('products', productsSheet, parseProducts);
...
function getProductSubcategories(products) {
  if (!Array.isArray(products)) return [];
  // Use Set for efficient unique values, filter out falsy values (empty strings, null), then sort

...
function movePaidRecords(masterSheet, paidSheet) {
    ...
    for (let i = masterData.length - 1; i >= 1; i--) {
        ...
        if (row[COL_PAIDFLAG - 1] === 'Y') {
            let amount = row[COL_AMOUNT - 1]
            let partialPaidAmount = row[COL_PARTIAL_PAID_AMOUNT - 1]
            let discountAmount = row[COL_DISCOUNT_AMOUNT - 1]
            if (partialPaidAmount && partialPaidAmount !== '') {
                if (amount > partialPaidAmount + discountAmount) {
                    masterSheet.getRange(i + 1, COL_PAIDFLAG).setValue('P');
                    continue; // Skip moving this record, it's partially paid
                }
            }
            let paidDate = new Date();
            let fileId = row[COL_FILEID - 1];
            masterSheet.getRange(i + 1, COL_FILEID, 1, 3).setValues([[paidDate, discountAmount, fileId]]);
...
function moveReceivePaidRecords(masterSheet, paidSheet) {
    let masterData = masterSheet.getDataRange().getValues();
    let move_files = [];
    ...
    for (let i = masterData.length - 1; i >= 1; i--) {
        ...
        if (row[COL_PAIDFLAG - 1] === 'Y') {
            ...
            let paidDate = new Date();
            let fileId = row[COL_FILEID - 1]; // Adjusted index for COL_FILEID
            ...
            // Prepare file move operation
            let yearFolder = getFolder(row[COL_YEAR - 1], ALL_RECEIVED_BILL_FOLDER_ID);
            let monthFolder = getFolder(row[COL_MONTH - 1], yearFolder.getId());
            let parentYearFolder = getFolder(row[COL_YEAR - 1], UPLOAD_RECEIVED_BILL_FOLDER_ID);
            let parentMonthFolder = getFolder(row[COL_MONTH - 1], parentYearFolder.getId());
...
function calculateYearSummary(masterData, paidData, discountData, discountBillData) {
    ...
    let discount_bill_filtered = discountBillData.filter(row => row[COL_YEAR - 1] !== '');
    ...
    let discountBillGroupByCode = groupBy(discount_bill_filtered, row => row[COL_CODE - 1]);
    let summary = [];
    ...
    for (let code in groupByCode) {
        ...
        let discountGroupByYear = discountGroupByCode[code]
            ...
            : {};
        let discountBillGroupByYear = discountBillGroupByCode[code]
            ? groupBy(discountBillGroupByCode[code], row => row[COL_YEAR - 1])
            : {};
        ...
        for (let year in groupByYear) {
            ...
            let discountBillYearRows = discountBillGroupByYear[year] || [];
            ...
            let discountBillPaidAmount = discountBillYearRows
                .filter(row => row[COL_PAIDFLAG - 1] === 'Y')
                .reduce((sum, row) => sum + parseFloat(row[COL_AMOUNT - 1]), 0);
            unpaidAmount -= discountBillPaidAmount;
            if(unpaidAmount < 0) unpaidAmount = 0;

            let discountBillUnpaidAmount = discountBillYearRows
                .filter(row => row[COL_PAIDFLAG - 1] !== 'Y')
                .reduce((sum, row) => sum + parseFloat(row[COL_AMOUNT - 1]), 0);

            let discountCount = discountYearRows.length + discountBillYearRows.length;
            let discountAmount = discountYearRows.reduce((sum, row) => sum + parseFloat(row[COL_DISCOUNT_AMOUNT - 1]), 0) + discountBillPaidAmount;

            summary.push([
                code, name, year, totalFiles, totalAmount,
                paidFiles, paidAmount, discountCount, discountAmount, discountBillUnpaidAmount,
                unpaidFiles, unpaidAmount
            ]);
        }
    }

    for (let code in discountGroupByCode) {
        if (groupByCode[code]) continue;

        let discountGroupByYear = groupBy(discountGroupByCode[code], row => row[COL_YEAR - 1]);
        let discountBillGroupByYear = discountBillGroupByCode[code]
            ? groupBy(discountBillGroupByCode[code], row => row[COL_YEAR - 1])
            : {};

        for (let year in discountGroupByYear) {
            let discountYearRows = discountGroupByYear[year] || [];
            let discountBillYearRows = discountBillGroupByYear[year] || [];
            let discountCount = discountYearRows.length + discountBillYearRows.length;
            ...
            summary.push([
                code, name, year, 0, 0,
                0, 0, discountCount, 0, discountRemaining,
                0, 0
            ]);
        }
    }
...
function calculateReceiveYearSummary(masterData, paidData, discountData) {
    let data_to_calculate = masterData.concat(paidData)
        .filter(row => row[COL_YEAR - 1] !== '');
    ...
    for (let code in groupByCode) {
        ...
        for (let year in groupByYear) {
            ...
            let paidAmount = paidRows.reduce((sum, row) => {
                ...
                let discountAmount = parseFloat(row[COL_DISCOUNT_AMOUNT - 1] || 0);
                if (row[COL_PAIDFLAG - 1] === 'Y') {
                    amount = parseFloat(row[COL_AMOUNT - 1]) - discountAmount;
    ...
    for (let code in discountGroupByCode) {
        ...
        for (let year in discountGroupByYear) {
            ...
            summary.push([
                code, name, year, 0, 0,
                0, 0, discountCount, discountRemaining,
                0, 0
            ]);
        }
    }
...
function generateMonthlyReceiveSummary(data, yearColMap) {
    let groupByMonth = groupBy(data, row => row[COL_MONTH - 1]);
    let summaryData = [];
    ...
    Object.keys(monthHeaderMap).forEach(month => {
        ...
        Object.keys(yearColMap).forEach(year => {
            ...
            let totalPaidFiles = paidRows.length;
            let totalAmount = yearRows.reduce((sum, row) => sum + parseFloat(row[COL_AMOUNT - 1]), 0);
            let totalPaidAmount = paidRows.reduce((sum, row) => {
                let amount = 0
                let discountAmount = parseFloat(row[COL_DISCOUNT_AMOUNT - 1] || 0);
                if (row[COL_PAIDFLAG - 1] === 'Y') {
                    amount = parseFloat(row[COL_AMOUNT - 1])
                } else if (row[COL_PAIDFLAG - 1] === 'P') {
                    amount = parseFloat(row[COL_PARTIAL_PAID_AMOUNT - 1] || 0) + discountAmount;
                }
                return sum + amount;
            }, 0);

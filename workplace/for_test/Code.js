...
function sendMessages(id, data, resend = false) {
  let sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Group ID");
  let [main_group_id, telegram_bot_token, telegram_chatid, send_line, send_telegram] = sh.getRange("B1:B5").getValues().flat()
  const messages = {
    line: [
      {
        type: 'text',
    ...
    telegram: {
      parse_mode: 'HTML',
      text: `<b>📍เลขใบงาน: ${data.work_id}</b>
    
    👉<b>สถานะ</b> : ${data.status}
    
    📅<b>วันที่</b> : ${data.date}
    <b>แหล่งที่มา</b> : ${data.source}
    <b>ผู้ส่ง</b> : ${data.sender}
    <b>สาขาทีลูกค้าสนใจ</b> : ${data.branch}
    <b>สินค้าที่สนใจ</b> : ${data.product}

    📁 <b>ข้อมูลลูกค้า</b>
    <b>ชื่อ-สกุล</b> : ${data.customer.fname} ${data.customer.lname}
    <b>เลข ปชช</b> : ${data.customer.nationid}
    <b>อายุ</b> : ${data.customer.age} ปี
    <b>เบอร์โทร</b> : ${data.customer.phone}
    <b>Line ID</b> : ${data.customer.lineid}
    <b>จังหวัด</b> : ${data.customer.province}
    <b>อาชีพ</b> : ${data.customer.occupation}
    <b>งบเงินดาวน์</b> : ${Number(data.customer.downpayment).toLocaleString()} บาท
    ${data.remark == '' ? '' : ('\n📝 <b>หมายเหตุ</b> : ' + data.remark + '\n')}
    👉 <a href="${update_url}?wid=${data.work_id}">คลิกอัพเดทข้อมูล</a>`
    }
  }
  if (resend) {
    messages.line[0].text = '⚠️⚠️⚠️ แจ้งเตือน ใบงานเลขที่ ' + data.work_id + ' ไม่มีการอัพเดทเกิน 2 วัน ⚠️⚠️⚠️\n\n' + messages.line[0].text
    messages.line.push({
      type: 'text',
      text: `📌 กรุณาอัพเดทสถานะของใบงานเลขที่ ${data.work_id} ในลิงค์ด้านบน`
    })
    messages.telegram.text = '⚠️⚠️⚠️ แจ้งเตือน ใบงานเลขที่ ' + data.work_id + ' ไม่มีการอัพเดทเกิน 2 วัน ⚠️⚠️⚠️\n\n' + messages.telegram.text + `\n\n📌 กรุณาอัพเดทสถานะของใบงานเลขที่ ${data.work_id} ในลิงค์ด้านบน`
  }
  if (send_line) LineBotWebhook.push(id, LINE_ACCESS_TOKEN, messages.line)
  if (send_telegram) {
    let telegram_url = `https://api.telegram.org/bot${telegram_bot_token}/sendMessage`
    UrlFetchApp.fetch(telegram_url, {
      method: 'post',

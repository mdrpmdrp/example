...
function createPdfForRow(sheet, iDataRow) {
  ...
  var colBZ = sheet.getRange('$BZ' + iDataRow).getValue(); //สถานะการพิจารณาคุณวุฒิ
  ...
  if (colBZ === "รอการพิจารณาคุณวุฒิ") {  //น้องอ่ะ ทำ status = ผ่าน ถึงจะส่ง mail อีก และ form อื่น ก็จะไม่ส่ง mail
    sendEmail(colY, pdfFile, colBK, colBL, colC, colD, colE, colBU)
  }
...
function sendEmail(email, pdfFile, colBK, colBL, colC, colD, colE, colBU) {
  var fullName = escapeHtml(colC + colD + ' ' + colE);
  var applicationType = escapeHtml(colBK);
  var positionName = escapeHtml(colBL);
  var applicationFee = escapeHtml(colBU);
  var subject = 'ใบสมัครงาน-' + colC + colD + ' ' + colE;
  var plainBody = [
    'เรื่อง ขอส่งใบสมัครและข้อมูลของผู้สมัครสอบ',
    '',
    'เรียน ' + colC + colD + ' ' + colE,
    'ท่านประสงค์จะสมัครเป็น ' + colBK + ' ตำแหน่งงาน ' + colBL,
    'กรุณารอการตรวจสอบคุณสมบัติทั่วไป และคุณสมบัติเฉพาะตำแหน่ง ก่อนชำระค่าสมัครสอบ',
    'หากผลการตรวจสอบคุณสมบัติ "ผ่าน" กรุณาชำระค่าสมัครสอบ เป็นเงิน ' + colBU + ' บาท',
    'และส่งหลักฐานการโอนเงิน พร้อมพิมพ์ใบสมัครมานำยื่นในวันสอบด้วย',
    '',
    'สอบถามเพิ่มเติมได้ที่งานทรัพยากรบุคคล',
    'โทร. 0 2562 0951-6 สายใน 622579',
    '',
    'จึงเรียนมาเพื่อโปรดทราบ'
  ].join('\n');
  var html = [
    '<div style="margin:0;padding:24px 0;background-color:#f4f7fb;font-family:Tahoma,Arial,sans-serif;color:#1f2937;">',
    '  <div style="max-width:680px;margin:0 auto;padding:0 16px;">',
    '    <div style="background:linear-gradient(135deg,#0f766e 0%,#155e75 100%);border-radius:24px 24px 0 0;padding:28px 32px;color:#ffffff;">',
    '      <div style="font-size:13px;letter-spacing:0.08em;text-transform:uppercase;opacity:0.85;">Recruitment Application</div>',
    '      <h1 style="margin:12px 0 0;font-size:28px;line-height:1.35;font-weight:700;">ขอส่งใบสมัครและข้อมูลของผู้สมัครสอบ</h1>',
    '    </div>',
    '    <div style="background-color:#ffffff;border:1px solid #dbe4ee;border-top:none;border-radius:0 0 24px 24px;padding:32px;box-shadow:0 20px 45px rgba(15,23,42,0.08);">',
    '      <p style="margin:0 0 20px;font-size:16px;line-height:1.8;">เรียน <strong style="color:#0f766e;">' + fullName + '</strong></p>',
    '      <p style="margin:0 0 24px;font-size:15px;line-height:1.8;">ระบบได้รับข้อมูลการสมัครของท่านเรียบร้อยแล้ว โดยท่านประสงค์จะสมัครเป็น <strong>' + applicationType + '</strong> ตำแหน่งงาน <strong>' + positionName + '</strong></p>',
    '      <div style="margin:0 0 24px;padding:20px;border-radius:18px;background-color:#f8fafc;border:1px solid #e2e8f0;">',
    '        <div style="font-size:14px;font-weight:700;color:#0f172a;margin-bottom:12px;">ข้อมูลสำคัญ</div>',
    '        <table role="presentation" style="width:100%;border-collapse:collapse;font-size:14px;line-height:1.7;">',
    '          <tr>',
    '            <td style="padding:6px 0;color:#64748b;width:140px;">ประเภทการสมัคร</td>',
    '            <td style="padding:6px 0;color:#0f172a;font-weight:600;">' + applicationType + '</td>',
    '          </tr>',
    '          <tr>',
    '            <td style="padding:6px 0;color:#64748b;">ตำแหน่งงาน</td>',
    '            <td style="padding:6px 0;color:#0f172a;font-weight:600;">' + positionName + '</td>',
    '          </tr>',
    '          <tr>',
    '            <td style="padding:6px 0;color:#64748b;">ค่าธรรมเนียม</td>',
    '            <td style="padding:6px 0;color:#0f172a;font-weight:600;">' + applicationFee + ' บาท</td>',
    '          </tr>',
    '        </table>',
    '      </div>',
    '      <div style="margin:0 0 24px;padding:22px;border-radius:18px;background-color:#ecfeff;border:1px solid #a5f3fc;">',
    '        <div style="font-size:14px;font-weight:700;color:#155e75;margin-bottom:12px;">ขั้นตอนถัดไป</div>',
    '        <ol style="margin:0;padding-left:20px;font-size:14px;line-height:1.9;color:#164e63;">',
    '          <li>รอการตรวจสอบคุณสมบัติทั่วไป และคุณสมบัติเฉพาะตำแหน่ง</li>',
    '          <li>ตรวจสอบสถานะจากบริการตรวจสอบสถานะใบสมัครก่อนชำระค่าสมัครสอบ</li>',
    '          <li>เมื่อสถานะเป็น "ผ่าน" ให้ชำระค่าสมัครสอบจำนวน ' + applicationFee + ' บาท</li>',
    '          <li>ส่งหลักฐานการโอนเงิน และพิมพ์ใบสมัครเพื่อนำมายื่นในวันสอบ</li>',
    '        </ol>',
    '      </div>',
    '      <div style="margin:0 0 24px;padding:18px 20px;border-left:4px solid #f59e0b;background-color:#fffbeb;border-radius:12px;">',
    '        <div style="font-size:14px;line-height:1.8;color:#92400e;">หากมีข้อสงสัย กรุณาติดต่อกลับได้ที่งานทรัพยากรบุคคล โทร. 0 2562 0951-6 สายใน 622579</div>',
    '      </div>',
    '      <p style="margin:0;font-size:15px;line-height:1.8;">จึงเรียนมาเพื่อโปรดทราบ</p>',
    '      <p style="margin:18px 0 0;font-size:15px;line-height:1.8;font-weight:700;color:#0f172a;">งานทรัพยากรบุคคล</p>',
    '    </div>',
    '  </div>',
    '</div>'
  ].join('');

  GmailApp.sendEmail(email, subject, plainBody, { bcc: 'supinya@ku.th', htmlBody: html, attachments: [pdfFile], name: 'งานทรัพยากรบุคคล' })
} 
...
function handleDirectCodeEdit(range) {
    ...
    const codeData = sheet.getRange('I1:I').getDisplayValues().flat();
...
function handleDateEdit(range) {
    ...
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName('รายการ');
    const codeData = sheet.getRange('I1:I').getValues().flat();
...
function handleDescriptionEdit(range) {
    ...
    sheet.getRange('H' + (index + 1)).setValue(value);
}
...
function recreateBidirectionalLinks() {
    ...
    const masterCodes = masterSheet.getRange('I1:I').getDisplayValues().flat();
    
    // Process all sheets in the spreadsheet
    ss.getSheets().forEach(sheet => {
        const sheetName = sheet.getName();
        if (['รายการ', 'Dashboard', 'ชีต3', 'ชีต4', 'Departments', 'รายการ Ecert', 'จำแนกฟอร์ม'].includes(sheetName)) return; // Skip master and specific sheets
        ...
        if (finder.length > 0) {
          if(finder[0].offset(0,1).getDisplayValue() == '') return
            finder.forEach(codeCell => {
                ...
                if (masterIndex >= 0) {
                    const masterA1 = 'I' + (masterIndex + 1);
                    ...
                    masterSheet.getRange('H' + (masterIndex + 1)).setValue(locationValue);
